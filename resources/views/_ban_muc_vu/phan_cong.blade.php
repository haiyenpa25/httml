    ...
    <script>
        $(function () {
            ...
            $('form').on('submit', function (e) {
                ...
                window.location.href = `{{ route('ban_muc_vu.phan_cong', ['ban_nganh_id' => $ban_nganh_id]) }}?month=${month}&year=${year}`;
            });
            ...
            $('.delete-btn').on('click', function () {
                ...
                if (confirm('Bạn có chắc chắn muốn xóa buổi nhóm này?')) {
                    $.ajax({
                        url: `{{ route('api.ban_muc_vu.delete_buoi_nhom', ['buoiNhom' => ':id']) }}`.replace(':id', id),
                        method: 'DELETE',
                        success: function (response) {
                            if (response.success) {
                                row.remove();
