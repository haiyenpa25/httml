            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body d-flex justify-content-between">
                        ...
                        <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modal-them-buoi-nhom">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-group mb-0">
                                            <select name="attendance[{{ $tinHuu->id }}][status]" class="form-control attendance-status">
                                                <option value="co_mat" {{ isset($diemDanhData[$tinHuu->id]) && $diemDanhData[$tinHuu->id]['status'] == 'co_mat' ? 'selected' : '' }}>Có mặt</option>
                                    ...
                                    <td>
                                        <div class="form-group mb-0">
                                            <input type="text" name="attendance[{{ $tinHuu->id }}][note]" class="form-control" placeholder="Ghi chú" value="{{ isset($diemDanhData[$tinHuu->id]) ? $diemDanhData[$tinHuu->id]['note'] : '' }}">
                                        </div>
                                    </td>
                                </tr>
                                @empty
            ...
            <form action="{{ route('api.ban_trung_lao.them_buoi_nhom') }}" method="POST" id="add-buoi-nhom-form">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="chu_de" placeholder="Nhập chủ đề buổi nhóm" required>
                    </div>
...
$(function () {
    ...
    // Add new buoi nhom form submission
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    alert('Buổi nhóm đã được tạo thành công!');
                    location.reload();
                } else {
                    alert('Lỗi: ' + response.message);
                }
    ...
    // Handle automatic form submission when changing buoi_nhom_id
    $('#buoi-nhom-select').on('change', function() {
        $('#filter-form').submit();
    });
    
    // Change row color based on attendance status
    ...
    // Trigger change event for existing status values
    $('.attendance-status').trigger('change');
    
    // Initialize PieChart if stats exist
