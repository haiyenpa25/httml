        ...
        $(function () {
            ...
            try {
                $('#ban-dieu-hanh-table').DataTable({
                    ...
                    responsive: true,
                    ajax: {
                        url: '{{ route("api.ban_nganh." . ($banType ?? "trung_lao") . ".dieu_hanh_list") }}',
                        type: 'GET',
                        data: function (d) {
                    ...
                    columns: [
                        ...
                        { data: 'stt', name: 'stt', orderable: false, searchable: false },
            ...
            try {
                $('#ban-vien-table').DataTable({
                    ...
                    responsive: true,
                    ajax: {
                        url: '{{ route("api.ban_nganh." . ($banType ?? "trung_lao") . ".ban_vien_list") }}',
                        type: 'GET',
                        data: function (d) {
                            d.ho_ten = $('#filter-ho-ten').val();
                            d.chuc_vu = $('#filter-chuc-vu').val();
                    ...
                    columnDefs: [
                        ...
                        { targets: [2, 3, 4], responsivePriority: 100 }
                    ],
                    order: [],
                    drawCallback: function (settings) {
            ...
            $('#filter-ho-ten, #filter-so-dien-thoai, #filter-dia-chi').on('keyup', function () {
                $('#ban-dieu-hanh-table').DataTable().ajax.reload();
                $('#ban-vien-table').DataTable().ajax.reload();
            });

            $('#filter-chuc-vu').on('change', function () {
                $('#ban-dieu-hanh-table').DataTable().ajax.reload();
                $('#ban-vien-table').DataTable().ajax.reload();
            });
            ...
            $('#confirm-delete').on('click', function () {
                ...
                $.ajax({
                    url: '{{ route("api.ban_nganh." . ($banType ?? "trung_lao") . ".xoa_thanh_vien") }}',
                    method: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}',
                        tin_huu_id: tinHuuId,
