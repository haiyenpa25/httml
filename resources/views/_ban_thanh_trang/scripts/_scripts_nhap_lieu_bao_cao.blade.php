    ...
    <script>
        $(function () {
            ...
            $('.update-count').on('click', function (e) {
                ...
                $.ajax({
                    url: '{{ route("_ban_thanh_trang.update_thamdu_trung_lao") }}',
                    type: 'POST',
                    data: data,
                    dataType: 'json',
                    beforeSend: function () {
            ...
            // Khởi tạo DataTable cho bảng Nhóm tối thứ 7 (Ban Thanh Tráng)
            try {
            ...
            $('#add-diem-manh-form').on('submit', function (e) {
                ...
                $.ajax({
                    url: '{{ route("_ban_thanh_trang.save_danhgia_trung_lao") }}',
                    method: 'POST',
                    data: formData,
                    dataType: 'json',
                    beforeSend: function () {
            ...
            $(document).on('click', '.remove-danh-gia', function () {
                ...
                $.ajax({
                    url: '{{ route("api.ban_thanh_trang.xoa_danh_gia", ["id" => "__ID__"]) }}'.replace('__ID__', id),
                    method: 'DELETE',
                    data: {
                        _token: '{{ csrf_token() }}'
                    },
            ...
            $(document).on('click', '.remove-kiennghi', function () {
                ...
                if (id != '0') {
                    ...
                    $.ajax({
                        url: '{{ route("api.ban_thanh_trang.xoa_kien_nghi", ["id" => "__ID__"]) }}'.replace('__ID__', id),
                        method: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
