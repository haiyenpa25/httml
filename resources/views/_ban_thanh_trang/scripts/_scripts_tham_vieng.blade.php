...
@section('page-scripts')
    <!-- resources/views/scripts/ban_thanh_trang/ban_thanh_trang_tham_vieng.blade.php -->
        ...
        $(function () {
            ...
            if ($('#btn-filter-history').length) {
                $('#btn-filter-history').on('click', function () {
                    ...
                    $.ajax({
                        url: '{{ route("api.ban_thanh_trang.filter_tham_vieng") }}',
                        method: 'GET',
                        data: {
                            from_date: fromDate,
                            to_date: toDate
            ...
            $(document).on('click', '.btn-xem-chi-tiet', function () {
                ...
                $.ajax({
                    url: '{{ route("api.ban_thanh_trang.chi_tiet_tham_vieng", ["id" => "__ID__"]) }}'.replace('__ID__', id),
                    method: 'GET',
                    dataType: 'json',
                    success: function (response) {
                        console.log('Success response:', response);
            ...
            if ($('#filter-time').length) {
                $('#filter-time').on('change', function () {
                    ...
                    $.ajax({
                        url: '{{ route("api.ban_thanh_trang.filter_de_xuat_tham_vieng") }}',
                        method: 'GET',
                        data: { days: days },
                        dataType: 'json',
                        success: function (response) {
