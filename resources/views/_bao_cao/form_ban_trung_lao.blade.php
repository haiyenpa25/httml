            ...
            <div class="card-body">
                <form id="filter-form" method="GET" action="{{ route('_bao_cao.form_ban_trung_lao') }}">
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="ban_nganh_id">Buổi nhóm:</label>
                                <select id="ban_nganh_id" name="ban_nganh_id" class="form-control">
                                    <option value="1" {{ $currentBanNganhId == 1 ? 'selected' : '' }}>Ban Trung Lão (Nhóm tối thứ 7)</option>
                                    <option value="20" {{ $currentBanNganhId == 20 ? 'selected' : '' }}>Hội Thánh (Nhóm Chúa Nhật)</option>
                                </select>
                            </div>
                        </div>
                ...
                <div class="tab-content" id="baocao-tabContent">
                    ...
                    <div class="tab-pane fade show active" id="buoinhom" role="tabpanel">
                        ...
                        <form id="thamdu-form" method="POST" action="{{ route('_bao_cao.save_thamdu_trung_lao') }}">
                            ...
                            <input type="hidden" name="ban_nganh_id" value="{{ $currentBanNganhId }}">
                            
                            <!-- Nhóm Chúa Nhật (nếu đang ở tab Hội Thánh) -->
                            @if($currentBanNganhId == 20)
                            <div class="card mb-4">
                                        ...
                                        <table class="table table-bordered table-hover mb-0">
                                            <thead class="thead-light">
                                                <tr>
                                                    <th width="5%">STT</th>
                                                    <th width="10%">Ngày</th>
                                                    <th width="15%">Đề tài</th>
                                                    <th width="15%">Kinh Thánh</th>
                                                    <th width="15%">Diễn giả</th>
                                                    <th width="15%">Số lượng Trung Lão</th>
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <input type="text" class="form-control money-format" 
                                                            ...
                                                            value="{{ $buoiNhom->giaoDichTaiChinh ? number_format($buoiNhom->giaoDichTaiChinh->so_tien, 0, ',', '.') : '0' }}"
                                                        >
                                                ...
                                                @empty
                                                <tr>
                                                    <td colspan="8" class="text-center">Không có dữ liệu buổi nhóm trong tháng này</td>
                                                </tr>
                                                @endforelse
                                            </tbody>
                                        </table>
                    ...
                    <div class="tab-pane fade" id="danhgia" role="tabpanel">
                        <form id="danhgia-form" method="POST" action="{{ route('_bao_cao.save_danhgia_trung_lao') }}">
                            ...
                            <input type="hidden" name="month" value="{{ $currentMonth }}">
                            <input type="hidden" name="year" value="{{ $currentYear }}">
                            <input type="hidden" name="ban_nganh_id" value="1"> <!-- Luôn là Ban Trung Lão -->
                    ...
                    <div class="tab-pane fade" id="kehoach" role="tabpanel">
                        <form id="kehoach-form" method="POST" action="{{ route('_bao_cao.save_kehoach_trung_lao') }}">
                            @csrf
                            <input type="hidden" name="month" value="{{ $nextMonth }}"> <!-- Tháng tiếp theo -->
                            <input type="hidden" name="year" value="{{ $nextYear }}"> <!-- Năm tiếp theo (nếu tháng hiện tại là 12) -->
                            <input type="hidden" name="ban_nganh_id" value="1"> <!-- Luôn là Ban Trung Lão -->
...
$(function() {
    ...
    // Cập nhật nhanh số lượng tham dự
    $('.update-count').on('click', function() {
        ...
        // Nếu có dâng hiến (chỉ ở Ban Trung Lão)
        if (row.find('input[name^="buoi_nhom['+buoiNhomId+'][dang_hien]"]').length) {
            let dangHien = row.find('input[name="buoi_nhom['+buoiNhomId+'][dang_hien]"]').val();
            // Xóa dấu phân cách
            dangHien = dangHien ? dangHien.replace(/\./g, '') : '0';
            data.dang_hien = dangHien;
        }
        
        $.ajax({
            url: '{{ route("_bao_cao.update_thamdu_trung_lao") }}',
            type: 'POST',
            dataType: 'json',
            data: data,
