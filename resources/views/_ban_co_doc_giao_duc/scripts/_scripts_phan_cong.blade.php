    ...
    <script>
        $(function () {
            ...
            $('form').on('submit', function (e) {
                ...
                window.location.href = `{{ route('_ban_co_doc_giao_duc.phan_cong') }}?month=${month}&year=${year}`;
            });
            ...
            $('.delete-btn').on('click', function () {
                ...
                if (confirm('Bạn có chắc chắn muốn xóa buổi nhóm này?')) {
                    $.ajax({
                        url: `{{ route('api.ban_co_doc_giao_duc.delete_buoi_nhom', ['buoiNhom' => ':id']) }}`.replace(':id', id),
                        method: 'DELETE',
                        success: function (response) {
                            if (response.success) {
                                row.remove();
