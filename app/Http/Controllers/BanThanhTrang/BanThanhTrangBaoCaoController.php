...
class BanThanhTrangBaoCaoController extends Controller
{
     ...
     */
    public function formBaoCaoBanThanhTrang(Request $request): View
    {
        ...
        $buoiNhomBTL = BuoiNhom::with(['dienGia', 'giaoDichTaiChinh'])
            ->whereYear('ngay_dien_ra', $year)
            ->whereMonth('ngay_dien_ra', $month)
            ->where('ban_nganh_id', self::BAN_THANH_TRANG_ID)
            ->orderBy('ngay_dien_ra')
            ->get();
        ...
        return view('_ban_thanh_trang.nhap_lieu_bao_cao', compact(
            'month',
            'year',
            'nextMonth',
            'nextYear',
     ...
     */
    public function baoCaoBanThanhTrang(Request $request)
    {
        ...
        $thamVieng = ThamVieng::with(['tinHuu', 'nguoiTham'])
            ...
            ->where('id_ban', self::BAN_THANH_TRANG_ID)
            ->orderBy('ngay_tham')
            ->get();
        ...
        $diemManh = DanhGia::where('ban_nganh_id', self::BAN_THANH_TRANG_ID)
            ->where('loai', 'diem_manh')
            ->where('thang', $month)
            ->where('nam', $year)
            ->get();

        $diemYeu = DanhGia::where('ban_nganh_id', self::BAN_THANH_TRANG_ID)
            ->where('loai', 'diem_yeu')
            ->where('thang', $month)
            ->where('nam', $year)
            ->get();

        $kienNghi = KienNghi::where('ban_nganh_id', self::BAN_THANH_TRANG_ID)
            ->where('thang', $month)
            ->where('nam', $year)
            ->get();
            ...
            if ($request->has('dang_hien') && $request->dang_hien !== null) {
                ...
                if ($dangHien > 0) {
                    $giaoDich = GiaoDichTaiChinh::firstOrNew([
                        ...
                        'ban_nganh_id' => self::BAN_THANH_TRANG_ID,
                    ]);
    ...
    public function saveThamDuTrungLao(Request $request)
    {
        ...
        try {
            foreach ($request->buoi_nhom as $id => $data) {
                ...
                if ($buoiNhom->ban_nganh_id == self::BAN_THANH_TRANG_ID && isset($data['dang_hien']) && $data['dang_hien'] !== null) {
                    // Chuẩn hóa giá trị dang_hien
     ...
     */
    public function luuBaoCaoBanThanhTrang(Request $request)
    {
        Log::info('luuBaoCaoBanThanhTrang: Nhận yêu cầu', $request->all());
        ...
        try {
            ...
            DB::commit();
            Log::info('luuBaoCaoBanThanhTrang: Lưu thành công');
            return response()->json([
                'success' => true,
            ...
            DB::rollBack();
            Log::error('luuBaoCaoBanThanhTrang: Lỗi lưu báo cáo tổng hợp', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            return response()->json([
