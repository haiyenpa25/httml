...
class BanMucVuThanhVienController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $banDieuHanh = Cache::remember("ban_nganh_{$ban_nganh_id}_dieu_hanh", now()->addHour(), function () use ($banNganh) {
        ...
        $banVien = Cache::remember("ban_nganh_{$ban_nganh_id}_ban_vien", now()->addHour(), function () use ($banNganh) {
            return TinHuuBanNganh::with('tinHuu')
                ->where('ban_nganh_id', $banNganh->id)
        ...
        return view('_ban_muc_vu.thanh_vien', compact('banNganh', 'banNganhs', 'banDieuHanh', 'banVien', 'tinHuuList'));
    }

    /**
     * Thêm thành viên vào Ban Mục Vụ
     */
    public function themThanhVien(Request $request): JsonResponse
    {
        ...
        if ($exists) {
            return $this->errorResponse('Thành viên này đã thuộc ban ngành!', 422);
        }
        ...
        try {
            ...
            $this->clearBanMucVuCache($validatedData['ban_nganh_id']);
            DB::commit();
            return $this->successResponse('Đã thêm thành viên vào ban ngành thành công!');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    /**
     * Xóa thành viên khỏi Ban Mục Vụ
     */
    public function xoaThanhVien(Request $request): JsonResponse
    {
        ...
        if (!$recordExists) {
            return $this->notFoundResponse('Không tìm thấy thành viên trong ban ngành để xóa.');
        }
        ...
        try {
            ...
            $this->clearBanMucVuCache($validatedData['ban_nganh_id']);
            DB::commit();
            return $this->successResponse('Đã xóa thành viên khỏi ban ngành thành công!');
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Lỗi khi xóa thành viên khỏi ban ngành: ' . $e->getMessage());
            return $this->errorResponse('Đã xảy ra lỗi khi xóa thành viên: ' . $e->getMessage(), 500);
        }
    }

    /**
     * Cập nhật chức vụ thành viên trong Ban Mục Vụ
     */
        ...
        try {
            ...
            if ($validatedData['chuc_vu'] === 'Trưởng Ban') {
                ...
                if ($existingTruongBan) {
                    return $this->errorResponse('Ban ngành đã có Trưởng Ban! Vui lòng thay đổi chức vụ của người hiện tại trước.', 422);
                }
    ...
    /**
     * Xóa cache liên quan đến Ban Mục Vụ
     */
    private function clearBanMucVuCache($ban_nganh_id): void
    {
        Cache::forget("ban_nganh_{$ban_nganh_id}");
        Cache::forget("ban_nganh_{$ban_nganh_id}_dieu_hanh");
        Cache::forget("ban_nganh_{$ban_nganh_id}_ban_vien");
        Cache::forget("ban_nganh_{$ban_nganh_id}_thanh_vien");
    }
    ...
    public function diemDanh(Request $request): View
    {
        ...
        return view('_ban_muc_vu.diem_danh', compact(
            'banNganh',
            'banNganhs',
            'months',
            'years',
    ...
    public function phanCong(Request $request): View
    {
        ...
        return view('_ban_muc_vu.phan_cong', compact(
            'banNganh',
            'banNganhs',
            'buoiNhoms',
            'dienGias',
    ...
    public function updateBuoiNhom(Request $request, BuoiNhom $buoiNhom): JsonResponse
    {
        ...
        try {
            $banNganh = BanNganh::findOrFail($buoiNhom->ban_nganh_id);
            if ($buoiNhom->ban_nganh_id !== $banNganh->id) {
                return $this->forbiddenResponse('Buổi nhóm không thuộc ban ngành này.');
            }
    ...
    public function phanCongChiTiet(Request $request): View
    {
        ...
        return view('_ban_muc_vu.phan_cong_chi_tiet', compact(
            'banNganh',
            'banNganhs',
            'months',
            'years',
    ...
    public function phanCongNhiemVu(Request $request): JsonResponse
    {
        ...
        try {
            ...
            if (!$isMember) {
                return $this->forbiddenResponse('Người được phân công không thuộc ban ngành này.');
            }

            $nhiemVu = NhiemVu::find($request->nhiem_vu_id);
            if ($nhiemVu->id_ban_nganh != $banNganh->id) {
                return $this->forbiddenResponse('Nhiệm vụ không thuộc ban ngành này.');
            }
     ...
     */
    public function dieuHanhList(Request $request, $ban_nganh_id): JsonResponse
    {
        try {
            $banNganh = BanNganh::findOrFail($ban_nganh_id);
     ...
     */
    public function banVienList(Request $request, $ban_nganh_id): JsonResponse
    {
        try {
            $banNganh = BanNganh::findOrFail($ban_nganh_id);
            $query = TinHuuBanNganh::with('tinHuu')
