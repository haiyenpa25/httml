...
class BaoCaoBanCoDocGiaoDucController extends Controller
{
    ...
    public function baoCaoBanThanhTrang(Request $request): View
    {
        ...
        $banDieuHanh = TinHuuBanNganh::with('tinHuu')
            ->where('ban_nganh_id', $banCoDocGiaoDucId)
        ...
        $thamVieng = ThamVieng::with(['tinHuu', 'nguoiTham'])
            ...
            ->where('id_ban', $banCoDocGiaoDucId)
            ->orderBy('ngay_tham')
            ->get();
        ...
        // 7. Lấy đánh giá
        $diemManh = DanhGia::where('ban_nganh_id', $banCoDocGiaoDucId)
            ->where('loai', 'diem_manh')
            ->where('thang', $month)
            ->where('nam', $year)
            ->get();

        $diemYeu = DanhGia::where('ban_nganh_id', $banCoDocGiaoDucId)
            ->where('loai', 'diem_yeu')
            ->where('thang', $month)
            ->where('nam', $year)
            ->get();
        ...
        // 8. Lấy kiến nghị
        $kienNghi = KienNghi::where('ban_nganh_id', $banCoDocGiaoDucId)
            ->where('thang', $month)
            ->where('nam', $year)
            ->get();
        ...
        return view('_ban_co_doc_giao_duc.bao_cao', compact(
            'month',
            'year',
            'banDieuHanh',
            'buoiNhomHT',
    ...
    public function formBaoCaoBanThanhTrang(Request $request)
    {
        ...
        return view('_bao_cao.form_ban_co_doc_giao_duc', compact(
            'currentMonth',
            'currentYear',
            'currentBanNganhId',
            'nextMonth',
